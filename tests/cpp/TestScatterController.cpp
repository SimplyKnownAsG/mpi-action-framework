...
MAF_TEST_ACTION(test_equal_mpi_size) {
    void run() override {
        ...
        std::vector<std::shared_ptr<maf::Action>> actions(size, maf::ActionFactory::Create<IncrementContextAction>());
...
MAF_TEST_ACTION(test_twice_mpi_size) {
    void run() override {
        ...
        int size = maf::MafComm::World->size;
        std::vector<std::shared_ptr<maf::Action>> actions(2 * size, maf::ActionFactory::Create<IncrementContextAction>());
        maf::ScatterController controller(actions);
